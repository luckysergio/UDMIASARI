        ...
        <div className="container mx-auto text-center">
          <div className="max-w-4xl mx-auto">
            <div className="inline-flex items-center gap-2 px-2 sm:px-3 py-1 rounded-full bg-white/10 backdrop-blur-sm border border-white/20 mb-3 sm:mb-6" data-aos="fade-down">
              ...
              <span className="text-white text-[10px] sm:text-xs md:text-sm font-medium">Produsen Bakso Terpercaya di Tangerang</span>
            </div>
...
const Logo = ({ size = "md", showText = true, className = "" }) => {
  ...
  const sizeClasses = {
    sm: { container: "w-7 h-7 sm:w-8 sm:h-8", text: "text-base sm:text-lg", tagline: "text-[8px] sm:text-[10px]", icon: "w-4 h-4 sm:w-5 sm:h-5" },
    md: { container: "w-8 h-8 sm:w-10 sm:h-10", text: "text-lg sm:text-xl", tagline: "text-[10px] sm:text-xs", icon: "w-5 h-5 sm:w-6 sm:h-6" },
    lg: { container: "w-10 h-10 sm:w-12 sm:h-12", text: "text-xl sm:text-2xl", tagline: "text-xs sm:text-sm", icon: "w-6 h-6 sm:w-7 sm:h-7" },
  };
    ...
    <div className={`flex items-center gap-2 sm:gap-3 group ${className}`}>
      ...
      `}>
        {!imgError ? (
          ...
          <span className={`${sizeClasses[size].icon} text-white font-bold`}>MS</span>
        )}
      </div>
      {showText && (
        <div className="shrink-0 min-w-0">
          <h1 className={`font-bold bg-linear-to-r from-indigo-400 to-purple-400 bg-clip-text text-transparent ${sizeClasses[size].text} whitespace-nowrap`}>
            UD. Mia Sari
          </h1>
          <p className={`text-slate-400 ${sizeClasses[size].tagline} whitespace-nowrap`}>
...
const LandingPage = () => {
  ...
  useEffect(() => {
    ...
    fetchLandingData();
    loadCartFromStorage();
  }, [selectedCategory, selectedJenis, searchTerm, sortBy, sortOrder, currentPage]);
  ...
  const statistics = [
    { icon: Package, value: landingData?.statistics?.total_products || 0, label: "Produk Tersedia", gradient: "from-blue-500 to-cyan-500" },
    { icon: ShoppingCart, value: landingData?.statistics?.total_transactions || 0, label: "Produk Terjual", gradient: "from-green-500 to-emerald-500" },
    { icon: Package, value: landingData?.statistics?.total_customers || 0, label: "Customer Aktif", gradient: "from-purple-500 to-pink-500" },
    { icon: Package, value: `${landingData?.statistics?.satisfied_clients || 0}%`, label: "Kepuasan Customer", gradient: "from-yellow-500 to-orange-500" },
  ];
  ...
  const features = [
    { icon: Truck, title: "Pengiriman Cepat", description: "Proses pengiriman yang cepat dan aman ke seluruh Tangerang", gradient: "from-indigo-500 to-purple-500" },
    { icon: Shield, title: "Garansi Halal", description: "100% halal dengan sertifikasi resmi", gradient: "from-green-500 to-emerald-500" },
    { icon: Award, title: "Berkualitas", description: "Bahan baku pilihan dengan kualitas terjamin", gradient: "from-amber-500 to-orange-500" },
  ];

  return (
    <div className="min-h-screen bg-linear-to-br from-slate-900 via-slate-800 to-slate-900 overflow-x-hidden">
      {/* Navbar */}
      <nav className="bg-slate-900/80 backdrop-blur-md border-b border-slate-700/50 sticky top-0 z-50 transition-all duration-300">
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              {isAuthenticated() && isCustomer && (
                ...
                >
                  ...
                  {getCartItemCount() > 0 && (
                    <span className={`absolute -top-1 -right-1 min-w-4 sm:min-w-5 h-4 sm:h-5 bg-linear-to-r from-indigo-600 to-purple-600 text-white text-[10px] sm:text-xs rounded-full flex items-center justify-center px-1 transition-all duration-300 ${cartAnimating ? 'scale-125' : 'scale-100'}`}>
                      {getCartItemCount()}
                    </span>
                  )}
                </button>
              ...
              {isAuthenticated() ? (
                <div className="flex items-center gap-2 sm:gap-3">
                  <div className="hidden xs:block">
                    ...
                    <p className="text-white text-xs sm:text-sm font-medium">{user?.name?.split(" ")[0]}</p>
                  </div>
                  {isAdminOrProduksi ? (
                    <Link
                      to="/dashboard"
              ...
              return (
                ...
                >
                  ...
                  <div className="relative bg-slate-800/30 backdrop-blur-sm rounded-2xl p-3 sm:p-6 text-center border border-slate-700/50 group-hover:border-indigo-500/30 transition-all duration-300">
                    <div className={`w-10 h-10 sm:w-14 sm:h-14 rounded-xl bg-linear-to-br ${stat.gradient} bg-opacity-20 flex items-center justify-center mx-auto mb-2 sm:mb-3 shadow-lg`}>
        ...
        <div className="container mx-auto px-3 sm:px-4">
          <div className="text-center mb-8 sm:mb-12" data-aos="fade-up">
            <div className="inline-flex items-center gap-2 px-2 sm:px-3 py-1 rounded-full bg-indigo-500/20 border border-indigo-500/30 mb-3 sm:mb-4">
              ...
              <span className="text-indigo-400 text-[10px] sm:text-sm font-medium">Best Seller</span>
            </div>
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-white mb-2 sm:mb-3">Produk Terlaris</h2>
            <p className="text-slate-400 text-sm sm:text-base max-w-2xl mx-auto px-2">Produk-produk terbaik pilihan pelanggan kami</p>
          </div>
          
          <div className="grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
            {loading ? (
              Array(4).fill(0).map((_, i) => (
                <div key={i} className="bg-slate-800/50 rounded-2xl border border-slate-700 animate-pulse overflow-hidden">
              ...
              <div className="col-span-full text-center text-slate-400 py-8">Belum ada data produk terlaris</div>
            ) : (
              landingData.top_products.slice(0, 4).map((product, idx) => (
                ...
                >
                  ...
                  <div className="p-3 sm:p-4">
                    ...
                    <p className="text-[10px] sm:text-sm text-slate-400 mb-2 sm:mb-3">{product.code}</p>
                    <div className="flex items-center justify-between">
                      <p className="text-emerald-400 font-bold text-base sm:text-xl">{formatRupiah(product.price)}</p>
                      <button
        ...
        <div className="container mx-auto px-3 sm:px-4">
          <div className="text-center mb-8 sm:mb-12" data-aos="fade-up">
            <div className="inline-flex items-center gap-2 px-2 sm:px-3 py-1 rounded-full bg-indigo-500/20 border border-indigo-500/30 mb-3 sm:mb-4">
              ...
              <span className="text-indigo-400 text-[10px] sm:text-sm font-medium">Katalog Produk</span>
            </div>
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-white mb-2 sm:mb-3">Semua Produk</h2>
            <p className="text-slate-400 text-sm sm:text-base max-w-2xl mx-auto px-2">Temukan berbagai produk berkualitas dari kami</p>
          </div>
          
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3 sm:gap-4 mb-6 sm:mb-8" data-aos="fade-up">
            ...
            <div className="flex flex-wrap gap-2 sm:gap-3">
              ...
              <select
                value={sortBy === "price" && sortOrder === "asc" ? "price_asc" : sortBy === "price" && sortOrder === "desc" ? "price_desc" : sortBy}
          ...
          <div className={`overflow-hidden transition-all duration-500 ease-in-out ${showFilters ? 'max-h-96 opacity-100 mb-6 sm:mb-8' : 'max-h-0 opacity-0'}`}>
          ...
          {loading ? (
            ...
            <div className="text-center py-12 sm:py-16">
              ...
              <h3 className="text-lg sm:text-xl font-semibold text-white mb-2">Tidak Ada Produk</h3>
              <p className="text-slate-400 text-sm">Produk yang Anda cari tidak ditemukan</p>
            </div>
          ) : (
            <>
              <div className="grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
                {products.map((product, idx) => (
                  ...
                  >
                    ...
                    >
                      ...
                      {!product.is_available && (
                        <div className="absolute inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center">
                          <span className="bg-red-500 text-white px-2 sm:px-3 py-0.5 sm:py-1 rounded-full text-[10px] sm:text-sm font-medium">Stok Habis</span>
                        </div>
                      )}
                      <div className="absolute inset-0 bg-linear-to-t from-slate-900 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
                    </div>
                    <div className="p-3 sm:p-4">
                      ...
                      <p className="text-[10px] sm:text-sm text-slate-400 mb-1.5 sm:mb-2">{product.code}</p>
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-emerald-400 font-bold text-base sm:text-xl">{formatRupiah(product.price)}</p>
                          <p className="text-[8px] sm:text-xs text-slate-500">Stok: {product.stock}</p>
                        </div>
                        <button
                          onClick={() => handleViewProduct(product)}
                          className="px-2 sm:px-3 py-1 sm:py-1.5 bg-indigo-600/20 text-indigo-400 rounded-lg hover:bg-indigo-600 hover:text-white transition-all duration-300 text-[10px] sm:text-sm font-medium"
              ...
              {totalPages > 1 && (
                <div className="mt-8 sm:mt-12 flex justify-center gap-1.5 sm:gap-2 flex-wrap" data-aos="fade-up">
                  <button
                    ...
                    return (
                      <button
                        ...
                        className={`w-8 h-8 sm:w-10 sm:h-10 rounded-xl font-medium transition-all duration-300 text-sm sm:text-base ${
                          currentPage === pageNum ? "bg-linear-to-r from-indigo-600 to-purple-600 text-white shadow-lg shadow-indigo-500/25" : "bg-slate-700/50 text-slate-300 hover:bg-slate-600/50"
                        }`}
                      >
                        {pageNum}
                      </button>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 sm:gap-12">
            <div data-aos="fade-right">
              <div className="inline-flex items-center gap-2 px-2 sm:px-3 py-1 rounded-full bg-indigo-500/20 border border-indigo-500/30 mb-3 sm:mb-4">
                ...
                <span className="text-indigo-400 text-[10px] sm:text-sm font-medium">Tentang Kami</span>
              </div>
              <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-white mb-3 sm:mb-4">Tentang UD. Mia Sari</h2>
              <p className="text-slate-300 text-sm sm:text-base mb-4 sm:mb-6 leading-relaxed">
                UD. Mia Sari adalah produsen bakso rumahan asal Tangerang yang berkomitmen menyajikan bakso berkualitas tinggi, halal, dan higienis. Mengkombinasikan resep tradisional dengan proses produksi modern, kami siap memenuhi kebutuhan pasokan bakso untuk konsumsi harian, mitra warung retail, hingga pedagang kuliner keliling.
              </p>
              <div className="space-y-3 sm:space-y-4">
                <div className="flex items-start gap-3 p-3 sm:p-4 bg-slate-800/30 rounded-xl border border-slate-700/50">
                  ...
                  <div>
                    <h4 className="text-white font-semibold text-sm sm:text-base mb-0.5 sm:mb-1">Visi</h4>
                    <p className="text-slate-400 text-xs sm:text-sm">Menjadi produsen bakso pilihan utama di Tangerang dan sekitarnya yang dikenal karena kualitas rasa, kebersihan, dan pelayanan terbaik.</p>
                  </div>
                </div>
                <div className="flex items-start gap-3 p-3 sm:p-4 bg-slate-800/30 rounded-xl border border-slate-700/50">
                  ...
                  <div>
                    <h4 className="text-white font-semibold text-sm sm:text-base mb-0.5 sm:mb-1">Misi</h4>
                    <ul className="text-slate-400 text-xs sm:text-sm space-y-0.5 sm:space-y-1">
                      <li className="flex items-start gap-1.5 sm:gap-2">
                        ...
                        <span>Memproduksi bakso dengan bahan baku daging pilihan yang segar dan berkualitas.</span>
                      </li>
                      <li className="flex items-start gap-1.5 sm:gap-2">
                        <CheckCircle className="w-3 h-3 sm:w-4 sm:h-4 text-emerald-400 mt-0.5 shrink-0" />
                        <span>Menjaga standar kebersihan dan higienitas yang tinggi di setiap tahapan produksi.</span>
                      </li>
                      <li className="flex items-start gap-1.5 sm:gap-2">
                        <CheckCircle className="w-3 h-3 sm:w-4 sm:h-4 text-emerald-400 mt-0.5 shrink-0" />
                        <span>Memberikan kemudahan akses pemesanan bagi pelanggan retail maupun mitra bisnis.</span>
                      </li>
                    </ul>
                  </div>
                </div>
            ...
            <div className="bg-linear-to-br from-slate-800/40 to-slate-800/20 rounded-2xl border border-slate-700/50 p-4 sm:p-6 backdrop-blur-sm" data-aos="fade-left">
              <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-6">
                ...
                <h3 className="text-lg sm:text-xl font-semibold text-white">Informasi Kontak</h3>
              </div>
              ...
              <div className="space-y-3 sm:space-y-4">
                <div className="flex items-center gap-3 p-2.5 sm:p-3 rounded-xl bg-slate-700/30">
                  ...
                  <span className="text-slate-300 text-sm sm:text-base">Tangerang, Banten, Indonesia</span>
                </div>
                <div className="flex items-center gap-3 p-2.5 sm:p-3 rounded-xl bg-slate-700/30">
                  <Phone className="w-4 h-4 sm:w-5 sm:h-5 text-indigo-400" />
                  <span className="text-slate-300 text-sm sm:text-base">+62 85886682496</span>
                </div>
                <div className="flex items-center gap-3 p-2.5 sm:p-3 rounded-xl bg-slate-700/30">
                  <Mail className="w-4 h-4 sm:w-5 sm:h-5 text-indigo-400" />
                  <span className="text-slate-300 text-sm sm:text-base">baksomiasari@gmail.com</span>
                </div>
                <div className="flex items-center gap-3 p-2.5 sm:p-3 rounded-xl bg-slate-700/30">
                  <Clock className="w-4 h-4 sm:w-5 sm:h-5 text-indigo-400" />
                  <span className="text-slate-300 text-sm sm:text-base">Didirikan tahun 2024</span>
                </div>
              </div>
              
              <div className="mt-4 sm:mt-6 pt-4 sm:pt-6 border-t border-slate-700">
                <h4 className="text-white font-medium mb-3 text-sm sm:text-base">Ikuti Kami</h4>
                <div className="flex gap-3">
                  <a href="https://facebook.com/udmiasari.bakso" target="_blank" rel="noopener noreferrer" className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-slate-700/50 flex items-center justify-center hover:bg-indigo-600 transition-all duration-300 group">
                    <FacebookIcon className="w-4 h-4 sm:w-5 sm:h-5 text-slate-300 group-hover:text-white" />
                  </a>
                  <a href="https://instagram.com/udmiasari.bakso" target="_blank" rel="noopener noreferrer" className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-slate-700/50 flex items-center justify-center hover:bg-indigo-600 transition-all duration-300 group">
        ...
        <div className="container mx-auto px-3 sm:px-4">
          <div className="text-center mb-8 sm:mb-12" data-aos="fade-up">
            <div className="inline-flex items-center gap-2 px-2 sm:px-3 py-1 rounded-full bg-indigo-500/20 border border-indigo-500/30 mb-3 sm:mb-4">
              ...
              <span className="text-indigo-400 text-[10px] sm:text-sm font-medium">Keunggulan</span>
            </div>
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-white mb-2 sm:mb-3">Mengapa Memilih Kami?</h2>
            <p className="text-slate-400 text-sm sm:text-base max-w-2xl mx-auto px-2">Kami memberikan solusi terbaik untuk kebutuhan bakso Anda</p>
          </div>
            ...
            {features.map((feature, idx) => {
              ...
              return (
                <div 
                  ...
                  data-aos-delay={idx * 100}
                >
                  <div className={`w-12 h-12 sm:w-16 sm:h-16 rounded-xl bg-linear-to-br ${feature.gradient} bg-opacity-20 flex items-center justify-center mx-auto mb-3 sm:mb-4 shadow-lg group-hover:scale-110 transition-transform duration-300`}>
      ...
      {showProductModal && selectedProduct && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/80 backdrop-blur-md">
          <div className="bg-slate-800 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-slate-800/95 backdrop-blur-sm flex items-center justify-between p-4 sm:p-6 border-b border-slate-700">
              <h3 className="text-lg sm:text-xl font-semibold text-white">Detail Produk</h3>
              <button
                onClick={() => setShowProductModal(false)}
                className="w-7 h-7 sm:w-8 sm:h-8 rounded-lg bg-slate-700 hover:bg-slate-600 text-slate-400 hover:text-white transition-all duration-300 flex items-center justify-center"
              >
            ...
            <div className="p-4 sm:p-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                <div className="bg-linear-to-br from-slate-700 to-slate-800 rounded-xl overflow-hidden h-48 sm:h-64 flex items-center justify-center">
                ...
                <div>
                  <h2 className="text-xl sm:text-2xl font-bold text-white mb-1 sm:mb-2">{selectedProduct.name}</h2>
                  <p className="text-slate-400 text-xs sm:text-sm mb-1 sm:mb-2">Kode: {selectedProduct.code}</p>
                  <div className="flex flex-wrap gap-1 sm:gap-2 mb-2 sm:mb-4">
                    {selectedProduct.category_name && (
                      <span className="text-[10px] sm:text-xs text-indigo-400 bg-indigo-500/20 px-1.5 sm:px-2 py-0.5 rounded-full">{selectedProduct.category_name}</span>
                    )}
                    {selectedProduct.jenis_name && (
                      <span className="text-[10px] sm:text-xs text-emerald-400 bg-emerald-500/20 px-1.5 sm:px-2 py-0.5 rounded-full">{selectedProduct.jenis_name}</span>
                    )}
                  </div>
                  <p className="text-2xl sm:text-3xl font-bold text-emerald-400 mb-3 sm:mb-4">{formatRupiah(selectedProduct.price)}</p>
                  <div className="mb-3 sm:mb-4">
                    <p className="text-slate-300 text-sm sm:text-base mb-1 sm:mb-2">Deskripsi:</p>
                  ...
                  <div className="flex gap-2 sm:gap-3 mt-4 sm:mt-6">
                    {selectedProduct.is_available ? (
                      <button
                        ...
                        className="flex-1 py-2 sm:py-2.5 bg-linear-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white rounded-xl transition-all duration-300 font-medium flex items-center justify-center gap-1 sm:gap-2 text-sm sm:text-base"
                      >
                        <ShoppingCart className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
                        {isAuthenticated() && isCustomer ? "Tambah ke Keranjang" : "Login"}
                      </button>
                    ) : (
                      <button disabled className="flex-1 py-2 sm:py-2.5 bg-slate-600 text-slate-400 rounded-xl cursor-not-allowed text-sm sm:text-base">
                        Stok Habis
                      </button>
                    )}
                  </div>
          ...
          <div className="bg-slate-800 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-slate-800/95 backdrop-blur-sm flex items-center justify-between p-4 sm:p-6 border-b border-slate-700">
              <div className="flex items-center gap-1.5 sm:gap-2">
                <ShoppingBag className="w-4 h-4 sm:w-5 sm:h-5 text-indigo-400" />
                <h3 className="text-lg sm:text-xl font-semibold text-white">Keranjang</h3>
              </div>
              <button onClick={() => setShowCart(false)} className="w-7 h-7 sm:w-8 sm:h-8 rounded-lg bg-slate-700 hover:bg-slate-600 text-slate-400 hover:text-white transition-all duration-300 flex items-center justify-center">
                <X className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
              </button>
            </div>
            ...
            <div className="p-4 sm:p-6">
              {cart.length === 0 ? (
                <div className="text-center py-8 sm:py-12">
                  <ShoppingBag className="w-16 h-16 sm:w-20 sm:h-20 text-slate-600 mx-auto mb-4" />
                  <h3 className="text-base sm:text-lg font-semibold text-white mb-2">Keranjang Kosong</h3>
                  <p className="text-slate-400 text-sm">Belum ada produk di keranjang Anda</p>
                  <button onClick={() => setShowCart(false)} className="mt-4 sm:mt-6 px-4 sm:px-6 py-2 sm:py-2.5 bg-linear-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white rounded-xl transition-all duration-300 text-sm">
                    Lanjut Belanja
                  </button>
                </div>
              ) : (
                <>
                  <div className="space-y-2 sm:space-y-3 max-h-96 overflow-y-auto pr-1 sm:pr-2 custom-scroll">
                    {cart.map((item, idx) => (
                      <div key={idx} className="flex flex-wrap sm:flex-nowrap items-center gap-2 sm:gap-4 p-2 sm:p-3 bg-slate-700/30 rounded-xl hover:bg-slate-700/50 transition-all duration-300">
                        <div className="w-10 h-10 sm:w-14 sm:h-14 rounded-lg bg-slate-700 flex items-center justify-center overflow-hidden shrink-0">
                          {item.image ? <img src={item.image} alt={item.product_name} className="w-full h-full object-cover" /> : <Package className="w-4 h-4 sm:w-6 sm:h-6 text-slate-500" />}
                        </div>
                        <div className="flex-1 min-w-0">
                          <h4 className="text-white font-medium text-xs sm:text-sm truncate">{item.product_name}</h4>
                          <p className="text-[8px] sm:text-xs text-slate-400">{item.product_code}</p>
                          <p className="text-emerald-400 text-xs sm:text-sm font-semibold mt-0.5">{formatRupiah(item.price)}</p>
                        </div>
                        <div className="flex items-center gap-1 sm:gap-2">
                          <button onClick={() => updateCartQty(item.product_id, item.qty - 1)} className="w-5 h-5 sm:w-7 sm:h-7 rounded-lg bg-slate-600 hover:bg-slate-500 flex items-center justify-center">
                            <Minus className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-white" />
                          </button>
                          <span className="text-white w-6 sm:w-8 text-center text-xs sm:text-sm font-medium">{item.qty}</span>
                          <button onClick={() => updateCartQty(item.product_id, item.qty + 1)} className="w-5 h-5 sm:w-7 sm:h-7 rounded-lg bg-slate-600 hover:bg-slate-500 flex items-center justify-center">
                            <Plus className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-white" />
                          </button>
                        </div>
                        <div className="text-right min-w-16 sm:min-w-24">
                          <p className="text-white font-semibold text-xs sm:text-sm">{formatRupiah(item.subtotal)}</p>
                        </div>
                        <button onClick={() => removeFromCart(item.product_id)} className="w-5 h-5 sm:w-7 sm:h-7 rounded-lg bg-red-500/20 hover:bg-red-500/30 text-red-400 flex items-center justify-center">
                  ...
                  <div className="mt-4 sm:mt-6 pt-3 sm:pt-4 border-t border-slate-700">
                    <div className="flex justify-between items-center mb-3 sm:mb-4">
                      ...
                      <span className="text-xl sm:text-2xl font-bold text-emerald-400">{formatRupiah(getCartTotal())}</span>
                    </div>
                    <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
                      <button onClick={() => setShowCart(false)} className="flex-1 py-2 sm:py-2.5 bg-slate-700/50 hover:bg-slate-600/50 text-white rounded-xl text-sm">
                        Lanjut Belanja
                      </button>
                      <button onClick={handleCheckout} className="flex-1 py-2 sm:py-2.5 bg-linear-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white rounded-xl flex items-center justify-center gap-1.5 sm:gap-2 font-medium text-sm">
                        <ShoppingCart className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
                        Checkout
                      </button>
                    </div>
