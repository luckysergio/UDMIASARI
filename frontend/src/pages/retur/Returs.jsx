...
const Returs = () => {
  ...
  // 🔥 Perbaikan handleApprove - refresh data setelah approve
    ...
    warning(
      ...
      async () => {
        ...
        if (result.success) {
          ...
          if (selectedRetur && selectedRetur.id === retur.id) {
            const updatedRetur = await returService.getReturDetail(retur.id);
            if (updatedRetur.success) {
              setSelectedRetur(updatedRetur.data);
              setIsModalOpen(true);
            }
          }
        } else {
  ...
  // 🔥 Perbaikan handleComplete - refresh data setelah complete
  ...
  // 🔥 Perbaikan submitSendReplacement - refresh data setelah kirim
  ...
  return (
    <MainLayout>
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div className="text-center md:text-left">
            ...
            <p className="text-slate-400 mt-1 text-sm md:text-base">
              Kelola retur dan penggantian barang
            </p>
          </div>
          <Button
            variant="primary"
      ...
      {loading ? (
        ...
        <Card className="bg-slate-800/50 backdrop-blur-sm border border-slate-700 text-center">
          <div className="text-center py-12">
            ...
            <p className="text-slate-400">Belum ada retur yang tercatat</p>
            <Button variant="primary" onClick={handleOpenCreateModal} className="mt-4 mx-auto">
              Buat Retur Baru
            </Button>
          </div>
        </Card>
        ...
        <>
          <div className="space-y-4">
            {returs.map((retur) => {
              ...
              return (
                <Card key={retur.id} className="bg-slate-800/50 backdrop-blur-sm border border-slate-700 hover:border-indigo-500/50 transition-all duration-300">
                  <div className="p-4">
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                      ...
                      <div className="flex-1 text-right">
                        ...
                        <div className="flex items-center justify-end gap-2 mt-2 flex-wrap">
                          ...
                          {retur.status === "approved" && retur.type === "exchange" && (
                            <button
          ...
          {!loading && totalPages > 1 && (
            <div className="mt-8 flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="text-sm text-slate-400 text-center sm:text-left">
                Menampilkan {(currentPage - 1) * 10 + 1} - {Math.min(currentPage * 10, totalReturs)} dari {totalReturs} data
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-70 backdrop-blur-sm">
          <div className="bg-slate-800 rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-slate-800 flex items-center justify-between p-6 border-b border-slate-700">
              ...
              <button onClick={() => setIsModalOpen(false)} className="text-slate-400 hover:text-white transition shrink-0">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="p-6 space-y-4">
                ...
                <div className="p-3 bg-slate-700/30 rounded-xl">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-white font-medium">{selectedRetur.transaction?.invoice_no || "-"}</p>
                    </div>
                  </div>
                </div>
                <div className="p-3 bg-slate-700/30 rounded-xl">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-white font-medium">{selectedRetur.transaction?.customer_name || "Umum"}</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              {/* Items Table */}
              <div>
                <h4 className="text-white font-medium mb-2">Produk yang Diretur</h4>
              ...
              <div className="flex justify-end pt-4 border-t border-slate-700">
                <div className="text-right">
                  ...
                  <p className="text-2xl font-bold text-emerald-400">{formatRupiahDisplay(selectedRetur.total_refund)}</p>
                </div>
              </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-3">
                    {selectedRetur.images.map((image, idx) => (
                      ...
                      >
                        <img src={getImageUrl(image.image)} alt={`Bukti ${idx + 1}`} className="w-full h-full object-cover" />
                      </button>
                    ))}
                  </div>
                </div>
              ...
              {/* 🔥 Action Buttons in Modal - Lengkap */}
              ...
              {selectedRetur.status === "approved" && selectedRetur.type === "exchange" && (
                <div className="flex justify-center gap-3 pt-4 border-t border-slate-700">
                  <Button
                    variant="primary"
                    onClick={() => {
      ...
      {/* Modal Reject */}
      {actionType === "reject" && selectedRetur && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-70 backdrop-blur-sm">
          <div className="bg-slate-800 rounded-2xl shadow-2xl w-full max-w-md">
            <div className="sticky top-0 bg-slate-800 flex items-center justify-between p-6 border-b border-slate-700">
              <h3 className="text-xl font-semibold text-white flex-1 text-center">Tolak Retur</h3>
              <button onClick={() => { setActionType(null); setSelectedRetur(null); }} className="text-slate-400 hover:text-white transition shrink-0">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="p-6 space-y-4">
              <p className="text-slate-300 text-center">
                Apakah Anda yakin ingin menolak retur <span className="font-semibold text-white">{selectedRetur.return_no}</span>?
              </p>
              <div>
              ...
              <div className="flex justify-center gap-3 pt-4">
                <Button variant="secondary" onClick={() => setActionType(null)}>Batal</Button>
                <Button variant="danger" onClick={submitReject} disabled={isSubmitting}>
                  {isSubmitting ? <Loader2 className="w-4 h-4 animate-spin" /> : "Ya, Tolak"}
                </Button>
              </div>
            </div>
          </div>
      ...
      {/* Modal Send Replacement */}
      {actionType === "replacement" && selectedRetur && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-70 backdrop-blur-sm">
          <div className="bg-slate-800 rounded-2xl shadow-2xl w-full max-w-md">
            <div className="sticky top-0 bg-slate-800 flex items-center justify-between p-6 border-b border-slate-700">
              <h3 className="text-xl font-semibold text-white flex-1 text-center">Kirim Barang Pengganti</h3>
              <button onClick={() => { setActionType(null); setSelectedRetur(null); }} className="text-slate-400 hover:text-white transition shrink-0">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="p-6 space-y-4">
              <p className="text-slate-300 text-center">
                Masukkan nomor resi pengiriman untuk retur <span className="font-semibold text-white">{selectedRetur.return_no}</span>
              </p>
              <div>
                <label className="block text-sm font-medium text-slate-300 mb-2 text-center">
              ...
              <div className="flex justify-center gap-3 pt-4">
                <Button variant="secondary" onClick={() => setActionType(null)}>Batal</Button>
                <Button variant="primary" onClick={submitSendReplacement} disabled={isSubmitting}>
      ...
      {createModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-70 backdrop-blur-sm">
          <div className="bg-slate-800 rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-slate-800 flex items-center justify-between p-6 border-b border-slate-700">
              <h3 className="text-xl font-semibold text-white flex-1 text-center">Buat Retur Baru</h3>
              <button onClick={() => setCreateModalOpen(false)} className="text-slate-400 hover:text-white transition shrink-0" disabled={isSubmitting}>
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="p-6 space-y-4">
              ...
              {!selectedTransaction ? (
                <div>
                  <label className="block text-sm font-medium text-slate-300 mb-2 text-center">Pilih Transaksi</label>
                  <select
                    onChange={(e) => {
                      const transaction = transactions.find((t) => t.id === parseInt(e.target.value));
                  ...
                  >
                    ...
                    {transactions.map((transaction) => (
                      <option key={transaction.id} value={transaction.id}>
                        {transaction.invoice_no} - {formatRupiahDisplay(transaction.grand_total)}
                      </option>
                    ))}
                  </select>
                  {loadingTransactions && (
                ...
                <>
                  ...
                  <div className="bg-indigo-500/10 rounded-xl p-4 border border-indigo-500/30">
                    <div className="flex justify-between items-center">
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-emerald-400 font-semibold">{formatRupiahDisplay(selectedTransaction.grand_total)}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  {/* Pilih Tipe Retur */}
                  <div>
                    <label className="block text-sm font-medium text-slate-300 mb-2 text-center">Tipe Retur</label>
                  ...
                  {/* Daftar Produk */}
                  <div>
                    <h4 className="text-white font-medium mb-2 text-center">Pilih Produk yang Diretur</h4>
                    ...
                    <div className="flex flex-wrap gap-3 mb-2">
                      {returImagePreviews.map((preview, idx) => (
                        <div key={idx} className="relative">
                          <img src={preview} alt={`Preview ${idx + 1}`} className="w-20 h-20 rounded-lg object-cover border border-slate-600" />
                          <button type="button" onClick={() => removeReturImage(idx)} className="absolute -top-2 -right-2 p-1 bg-red-500 rounded-full text-white hover:bg-red-600 transition">
                            <X className="w-3 h-3" />
                          </button>
                        </div>
                      ))}
                      <label className="w-20 h-20 rounded-lg bg-slate-700 border-2 border-dashed border-slate-500 flex flex-col items-center justify-center cursor-pointer hover:border-indigo-500 transition">
                        ...
                        <input ref={fileInputRef} type="file" accept="image/*" multiple onChange={handleReturImageChange} className="hidden" />
                      </label>
                    </div>
                  ...
                  {returType === "refund" && (
                    <div className="bg-indigo-500/10 rounded-xl p-4 border border-indigo-500/30 text-center">
                      <p className="text-sm text-slate-400">Total Refund</p>
                      <p className="text-2xl font-bold text-emerald-400">{formatRupiahDisplay(calculateTotalRefund())}</p>
                    </div>
                  )}
                </>
              )}
              ...
              <div className="flex justify-center gap-3 pt-4">
                <Button variant="secondary" onClick={() => setCreateModalOpen(false)} disabled={isSubmitting}>
                  Batal
                </Button>
                {selectedTransaction && (
                  <Button variant="primary" onClick={submitCreateRetur} disabled={isSubmitting} className="min-w-25 flex items-center justify-center gap-2">
                    {isSubmitting ? (
                      <>
